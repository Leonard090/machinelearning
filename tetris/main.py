...
def animate_line_clear(surface, grid, cleared_lines, points_earned, lines_cleared_now):
    ...
    mid = len(grid[0]) // 2  # Number of columns
    steps = mid + 1
    ...
    for step in range(steps):
        ...
        # Draw the animated grid (do not redraw the original)
    ...
    # Shift all lines above the cleared ones down TOGETHER
    cleared_lines = sorted(cleared_lines)
    for line in reversed(cleared_lines):
