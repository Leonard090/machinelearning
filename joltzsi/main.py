...
def check_lines():
    ...
    if rows_cleared > 0 or cols_cleared > 0:
        ...
        # Animate clearing rows (center out)
        if rows_to_clear:
            steps = GRID_SIZE // 2 + 1
            delay = 80  # ms
        ...
        # Animate clearing columns (center out)
        if cols_to_clear:
            steps = GRID_SIZE // 2 + 1
            delay = 80  # ms
            for step in range(steps):
        ...
        points_earned = (rows_score + cols_score) * level
        score_colors = {
            1: (255, 165, 0),   # Orange
            2: (255, 255, 0),   # Yellow
            3: (0, 255, 0),     # Green
        ...
        while pygame.time.get_ticks() - popup_start < 1000:
            ...
            draw_score()
            draw_highlight(current_piece, piece_x, piece_y)
            draw_center_text(screen, f"+{points_earned}", big_font, score_color)
            pygame.display.flip()
            pygame.time.delay(50)

        # Update global cleared lines count and level up if needed
